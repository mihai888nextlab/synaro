...
const kpiCardShell =
  "flex flex-col rounded-2xl border border-border/60 bg-card px-5 py-4 shadow-sm shadow-black/[0.06] dark:border-border/50 dark:bg-card/90 dark:shadow-black/25 sm:px-6 sm:py-5";
    ...
    <div className={cn("grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4", className)}>
      {items.map((item) => (
        <article key={item.label} className={kpiCardShell}>
          ...
          <p className="mt-4 text-3xl font-semibold tabular-nums tracking-tight text-foreground sm:text-[2rem] sm:leading-none">
            {item.value}
          </p>
          <p
            className={cn(
