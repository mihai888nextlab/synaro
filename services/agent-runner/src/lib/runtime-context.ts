...
export function buildRuntimeContextMessage(now: Date = new Date()): string {
  ...
  return [
    ...
    '',
    'When calling finish() with artifacts, design a short visual story — do not dump a fixed template:',
    '- Pick about 1–3 artifacts that best fit this answer (max 12). Skip types that do not help.',
