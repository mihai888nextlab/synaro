...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === "GET") {
    try {
      const url = new URL(`${agentServiceUrl()}/api/agents/${encodeURIComponent(agentId)}/memory`);
      url.searchParams.set("userId", session.user.id);
      const upstream = await fetch(url.toString(), { headers: agentHeaders() });
  ...
  if (req.method === "DELETE") {
    try {
      const upstream = await fetch(
        `${agentServiceUrl()}/api/agents/${encodeURIComponent(agentId)}/memory`,
        {
          method: "DELETE",
