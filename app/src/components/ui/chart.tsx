...
/** Props Recharts injects into custom tooltip content (not the same as `<Tooltip />` root props in v3). */
...
export type ChartTooltipContentProps = Omit<React.ComponentProps<"div">, "payload"> & {
  active?: boolean;
          ...
          {payload.map((item: ChartTooltipPayloadItem, index: number) => {
            ...
            const nestedFill =
              item.payload && typeof item.payload === "object" && "fill" in item.payload
                ...
                : undefined;
            const indicatorColor = color || nestedFill || item.color || item.fill;
            ...
            return (
              ...
              >
                ...
                (item.name !== undefined || item.dataKey !== undefined) ? (
                  formatter(
                    item.value,
